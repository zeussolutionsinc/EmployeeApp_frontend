...
export default function SetH1bV3({ fieldsDisabled, retrievedFormData }) {
  ...
  // Opening the snackbar to display the alert messages
  ...
  const handleFileChange = async (event) => {
    ...
    if (file) {
      ...
      try {
        // EXPIRES MAY 31 2024
        const sasToken =
          "sv=2022-11-02&ss=bf&srt=sco&sp=rwdlaciytfx&se=2024-05-31T05:38:57Z&st=2024-05-05T21:38:57Z&spr=https&sig=pfp%2FzLRULWHxKsYIbAIuKgrhQpMmQs3T3GIxwaXT8RY%3D";
  ...
  // Fires up whever the user enters a values
  // CHanges the format of the data as expected by the backend
  ...
  // Handles change in the telephone number
  // since it need to explictly handle 2 values
  ...
  //     const method = location.state?.data?.registrationId ? "PUT" : "POST";

  //     // Construct URL based on method and registration ID
  //     const url = `https://zeusemployeeportalbackend.azurewebsites.net/api/H1b${
  //       location.state?.data?.registrationId
  //         ? `/${location.state?.data?.registrationId}`
  //         : ""
  //     }`;
  ...
  // const url = `https://localhost:7078/api/H1b${
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (error.name === "ValidationError") {
        ...
        // Stops execution to prevent submission
        return;
      }
      throw error; // Rethrow other types of errors to be caught by the outer catch block
    }
